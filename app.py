...
def index():
    ...
    )  # Home page with the stored username
...
def start_test():
    ...
    categories = request.form.getlist(
        "categories"
    )  # Get the selected categories from the POST form that redirected here
    questions = generate_test(
        categories
    )  # Generates the test by the categories in the utilis.py module
    session["questions"] = (
        questions  # Saves the generated question in the temporary data to be
        # later used by "/test"
    )
    session["current_question"] = 0  # Resets all variables to 0 at start of
    # test
    session["attempts"] = 0
    session["show_answer"] = 0
    session["score"] = 0
    return redirect(url_for("test"))  # After test has been created, start
    # the test
...
def test():
    ...
    questions = session[
        "questions"
    ]  # Get back the questions previously generated from /start_test
    current_question = session.get("current_question", 0)
    # Get current_question index (an integer the keeps track of the questions
    # answered) If "current_question" can't be found, it assumed the default
    # is 0 and the test has just been generated.

    correct_answer = questions[current_question][
        "answer"
    ]  # The answer of the current question being answered.
    try:  # If there is a 2nd correct answer (due to fraction
        # simplification), retrieve it
        correct_answer_2 = questions[current_question]["answer_second"]
    except Exception as e:
        # Occurs when there is no 2nd answer. This makes the variable
        # accordingly reduntant by making it equal to answer.
        correct_answer_2 = correct_answer
    session["show_answer"] = 0
    # Once a new question is displayed on 1st attempt, the answer mustn't be
    # shown, likewise with the feeback.
    feedback = None

    if (
        request.method == "POST"
    ):  # Only occurs if a POST request redirects, i.e., it is not the 1st
        # question
        ...
        if not (pattern.match(user_answer)) or (
            ...
            # attempt as an attempt, and alerts the user that they entered
            # ABNORMAL data.
        ...
        if user_answer == correct_answer or user_answer == correct_answer_2:
            ...
            if session["attempts"] <= 1:  # Only on the 1st or 2nd attempt.
                session[
                    "score"
                ] += 1  # Only count as "correct" in CSV on 1st, 2nd, but not
                # the 3rd attempt
                save_user_data(
                    questions[current_question],
                    bool(
                        (user_answer == correct_answer)
                        | (user_answer == correct_answer_2)
                    ),
                    time.time(),
                )
                # Add to CSV
            ...
            session[
                "current_question"
            ] += 1  # Counter increments as its the next question.
            if session["current_question"] >= len(
                questions
            ):  # All questions have been answered. Test is finished. Results
                # page is rendered.
                session[
                    "current_question"
                ] -= 1  # Accounts for the fact that after a correct attempt
                # the counter is incremented by one too much.
                return redirect(url_for("results"))  # Results page is rendered.
            return redirect(url_for("test"))
            # This only occurs if the prior "if" statement didn't occur,
            # i.e., the user did enter the correct answer, but it was (
            # continuing from previous line) given by hints beyond the 3rd
            # attempt. CSV has already been recorded as false answe. No
            # further CSV actions required.
        else:  # Answer was false
            session["attempts"] += 1  # increment attempt counter
            if (
                session["attempts"] >= 2
            ):  # on the 2nd or beyond attempt, the answer was wrong again.
                # Answer is shown. Result recorded as false
                # in CSV.
            ...
            else:  # On the 1st attempt, the answer was wrong. A hint is
                # given, but not the answer. No result
                # recorded yet in csv.
                feedback = {
                    "general": "That's not quite right",
            ...
            # Gives user another attempt

    # This only happens if no other "return" occured, i.e., 1st question and
    # no attempts.
    return render_template(
        "test.html",
        question=questions[current_question],
        show_answer=session.get("show_answer", 0),
    )
...
def results():
    ...
    if "questions" not in session:
        return redirect(
            url_for("index")
        )  # Error handling, redirects to home. This happens if user
        # misnavigates to this page without completing
        # question.
...
def import_csv():
    ...
    if file and allowed_file(file.filename):
        ...
        if mime_type != "text/csv":
            ...
            # Goes back to home page and tells user of the issue (incorrect
            # file formatting)
        ...
        if (
            ...
            and "correct" in df.columns
            and "timestamp" in df.columns
        ):
            df.to_csv(
                data_file, mode="w", header=headers, index=False
            )  # Correct Formatting
        else:
            return render_template(
                "index.html", CSV_ERROR=True, username=read_username()
            )
        ...
        # Goes back to home page and tells user of the issue (incorrect file
        # formatting)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "production" or True:
        ...
        flask_thread.daemon = True  # Ensure Flask exits when the main thread
        # exits
        flask_thread.start()
